        ...
        try:
            if not os.path.isdir(f"/tmp/{repoNameShort}"):
                ...
                if os.path.isdir(f'/tmp/{prevRepoName}'):
                    ...
                    shutil.rmtree(f'/tmp/{prevRepoName}',onerror=onerror)
            ...
            print("reached test")
            #code
            os.system(f"python3 -m flake8 --select=DUO /tmp/{repoNameShort} > /home/joshuasumarlin/dlint/{row['cve_id']}+{repoNameShort}+{row['hash']}.txt")
            cur.execute(
            f"""
